...
class UpdateParticipantSectionalIdTest {
	...
	@Test void updateParticipantSectionalIdTestA() {		
		
		model.commitTransfers();
		
		int[] expectedId = {701, 702, 703, 704, 705, 706, 707};
		
		ArrayList<School> actual = model.getFeederSchools(model.getSelectedEvent());
		int[] actualId = new int[actual.size()];
		
		for(int i = 0; i < actual.size(); i++) {
			actualId[i] = actual.get(i).getId();
		}
